...
//   LOG(ERROR) << "BEGIN delete form value table-1******************************";
//   // Construct query plan
//   TableInfo *table_info;
//   GetExecutorContext()->GetCatalog()->GetTable("table-1", table_info);
//   const Schema *schema = table_info->GetSchema();
...
//   auto index_scan_plan = std::make_shared<IndexScanPlanNode>(out_schema,table_info->GetTableName(),vector<IndexInfo*>{index_info},false,predicate);

//   std::vector<Row> result_set;
//   GetExecutionEngine()->ExecutePlan(index_scan_plan, &result_set, GetTxn(), GetExecutorContext());

//   // Verify
//   // ASSERT_EQ(result_set.size(), 1);
//   for (const auto &row : result_set) {
//     ASSERT_TRUE(row.GetField(0)->CompareLessThan(Field(kTypeInt, 50)));
//   }
//   // DELETE FROM table-1 WHERE id == 50
...
//   LOG(ERROR) << "BEGIN inssert value table-1******************************";
...
TEST_F(ExecutorTest, SimpleUpdateTest) {
  LOG(ERROR) << "BEGIN UPDATE table-1******************************";
